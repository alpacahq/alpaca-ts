...
function account(input: RawAccount): Account {
  try {
    return {
      ...input,
      raw: () => input,
...
function clock(input: RawClock): Clock {
  try {
    return {
      raw: () => input,
...
function order(input: RawOrder): Order {
  try {
    return {
      ...input,
      raw: () => input,
      ...
      time_in_force: input.time_in_force as OrderTimeInForce,
...
function orders(input: RawOrder[]): Order[] {
  try {
...
function position(rawPosition: RawPosition): Position {
  try {
    return {
      ...
      unrealized_intraday_plpc: parseFloat(
...
function positions(input: RawPosition[]): Position[] {
  try {
...
function tradeActivity(input: RawTradeActivity): TradeActivity | undefined {
  try {
    return {
      ...input,
      raw: () => input,
      ...
      qty: parseFloat(input.qty),
      side: input.side as TradeActivitySide,
...
function activities(input: Array<RawActivity>): Array<Activity> {
  try {
    return input
      .map((value) =>
        value.activity_type === 'FILL'
          ? tradeActivity(value)
